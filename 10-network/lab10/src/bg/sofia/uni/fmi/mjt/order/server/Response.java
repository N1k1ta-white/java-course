...
public record Response(Status status, String additionalInfo, Collection<Order> orders) {
    ...
    public static Response create(int id) {
        return new Response(Status.CREATED, String.format("ORDER_ID=%d", id), null);
    }
    ...
    public static Response decline(String errorMessage) {
        return new Response(Status.DECLINED, String.format("invalid: %s", errorMessage), null);
    }
    ...
    public String toString() {
        return switch (status) {
            case OK -> "{\"status\":\"" + status.toString() + "\", \"orders\":[" +
                    orders.stream()
                            .map(Order::toString)
                            .collect(Collectors.joining(", ")) + "]}";
            default -> "{\"status\":\"" + status.toString() + "\", \"additionalInfo\":\"" + additionalInfo + "\"}";
        };
    }
}