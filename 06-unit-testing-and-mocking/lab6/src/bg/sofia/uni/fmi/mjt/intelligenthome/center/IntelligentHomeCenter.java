...
public class IntelligentHomeCenter {
    ...
    public void register(IoTDevice device) throws DeviceAlreadyRegisteredException {
        ...
        if (storage.exists(device.getId())) {
            throw new DeviceAlreadyRegisteredException("That device already exist!");
        }
    ...
    public void unregister(IoTDevice device) throws DeviceNotFoundException {
        ...
        if (!storage.exists(device.getId())) {
            throw new DeviceNotFoundException("Device doesn't exist in storage!");
        }
    ...
    public int getDeviceQuantityPerType(DeviceType type) {
        if (type == null) {
            throw new IllegalArgumentException("Invalid argument null in DeviceType!");
        }
        int quantity = 0;
